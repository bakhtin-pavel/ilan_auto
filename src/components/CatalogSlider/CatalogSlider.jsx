...
const CatalogSlider = ({ item }) => {
    ...
    const handleMiniatureImageClick = (index) => {
        if (!slider1) return;
        ...
        <div className={styles.catalogSliderWrapper}>
            <div className={styles.slider} ref={sliderWrapperRef}>
                ...
                <div className={styles.sliderImages}>
                    {autoInfo.images.length === 1 ? (
                        <div style={{ width: sliderWidth }}>
                            <img
                                className={styles.mainImage}
                                src={autoInfo.images[0]}
                                alt=""
                            />
                        </div>
                    ) : (
                        ...
                        (autoInfo.images.length > 3 ? (
                            ...
                            >
                                {autoInfo.images.map((item, index) => (
                                    <img
                                        key={index}
                                        className={activeSlideIndex === index ? [styles.secondImage, styles.activeSecondImage].join(' ') : styles.secondImage}
                                        src={item}
                                        alt=""
                                    />
                                ))}
                            </Slider>
                        ) : autoInfo.images.length < 2 ? null : (
                            <div
                                style={{ width: sliderWidth }}
                                className={styles.miniaturesWrapper}
                            >
                                {autoInfo.images.map((item, index) => (
                                    <img
                                        key={index}
                                        className={activeSlideIndex === index ? [styles.secondImage, styles.activeSecondImage].join(' ') : styles.secondImage}
                                        src={item}
                                        onClick={() => handleMiniatureImageClick(index)}
                                        alt=""
                                    />
                                ))}
                            </div>
                        ...
                        <div className={styles.statsWrap}>
                            <FigureBlock className={styles.diagrams}>
                                <FigureBlock
                                    ...
                                    style={{
                                        width:
                                            calculateProgressLineWidth(0, autoInfo.volume, 6) + "%",
                                    }}
                                ></FigureBlock>
                            </FigureBlock>
                            <span className={styles.value}>
                                {`${formateDigitsString(autoInfo.volume, 1, 1)} л.`}
                            </span>
                        </div>
                    </div>
                    <div className={styles.statsItem}>
